...
nhn.husky.SE_SCharacter = $Class({
	...
	$ON_TOGGLE_SCHARACTER_LAYER : function(){
		...
		this.oApp.exec("TOGGLE_TOOLBAR_ACTIVE_LAYER", [this.elDropdownLayer, "sCharacter"]);
	},
...
//		this.oApp.exec("ATTACH_HOVER_EVENTS", [cssquery(">LI>BUTTON", this.aSCharList[i])]);
	},

	_stopBrowserEvent : function(obj, sEvent){
		$Fn(function(we){we.stop();}, this).attach(obj, sEvent);
	}
});