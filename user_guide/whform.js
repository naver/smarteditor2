...
var gbWhForm=false;
var gnMaxRslt = 10 ;
var sEnableAndSearch = "";
var bEnableAndSearch = false;
var bGiveAndSearch = false;
...
function getFormHTML(paneName)
{
	var sForm = "";
	sForm += "<table class=\"clsFormBackground\" width=\"100%\" cellspacing=\"0\" cellpadding=\"4\" border=\"0\">";
	sForm += "<form name=\"" + gstrFormName + "\" method=\"POST\" action=\"javascript:inputSubmit()\" style=\"width:100%\">";
	sForm += "<tr>";
	sForm += "<td>";
	if (paneName == "fts") {
	    if (gbSafari3)
	        sForm += "<p class=title>" + gsTitle + "<br><table width=\"100%\"><tr valign=\"middle\"><td width=\"100%\"><input class=\"inputfield\" type=\"text\" name=\"keywordField\" onkeypress=\"lookupKeyDown(event);\" onfocus=\"inputEnable(1);\"></td>";
	    else
	        sForm += "<p class=title>" + gsTitle + "<br><table width=\"100%\"><tr valign=\"middle\"><td width=\"100%\"><input class=\"inputfield\" type=\"text\" name=\"keywordField\" onkeypress=\"lookupKeyDown(event);\" onfocus=\"inputEnable(1);\" onblur=\"inputEnable(0);\"></td>";
	}
	else
	    sForm += "<p class=title>" + gsTitle + "<br><table width=\"100%\"><tr valign=\"middle\"><td width=\"100%\"><input class=\"inputfield\" type=\"text\" name=\"keywordField\" onkeyup=\"lookupKeyDown(event);\" onfocus=\"inputEnable(1);\" onblur=\"inputEnable(0);\"></td>";
    ...
    if (paneName == "fts") {
        if (bGiveAndSearch) {
            if (bEnableAndSearch) {
                //Check box will checked
                sForm += "</tr><tr class=\"hilite\" valign=\"middle\"><td width=\"100%\"><input type=\"checkbox\" name=\"EnableAndSearch\" checked>"
                                  + sEnableAndSearch + "<br></td>";
            }
            else {
                sForm += "</tr><tr class=\"hilite\" valign=\"middle\"><td width=\"100%\"><input type=\"checkbox\" name=\"EnableAndSearch\" >"
                                  + sEnableAndSearch + "<br></td>";
            }
        }
    }
...
function input_filter(a_sVal)
{
	...
	var oMsg = new whMessage(WH_MSG_SETMAXRSLT, num, null);
	notify(oMsg);
	return num ;
}
...
function setSearchSetting(a_bGiveAndSearch, a_bEnable, a_strEnableAndSearch)
{
    bGiveAndSearch = a_bGiveAndSearch;
    bEnableAndSearch = a_bEnable;
    sEnableAndSearch = a_strEnableAndSearch;
}
